...
import net.runelite.client.plugins.microbot.ksptutisland.areas.TutAreas;
...
public class KSPTutIslandScript extends Script
{
    ...
    private boolean isInStartArea()
    {
        ...
        return location != null && TutAreas.START_AREA.contains(location);
    }
    ...
    {
        if (!walkToArea(TutAreas.SURVIVAL_AREA)) {
            return false;
        }
    ...
    private void cookingGuide()
    {
        ...
        if (progress == 120) {
            ...
            walkToArea(TutAreas.COOKING_AREA);
    ...
    private void questGuide()
    {
        ...
        if (progress == 200 || progress == 210) {
            walkToArea(TutAreas.QUEST_TUT_AREA);
    ...
    private void miningGuide()
    {
        ...
        if (Microbot.getVarbitPlayerValue(281) == 260) {
            walkToArea(TutAreas.MINING_SMITHING_AREA);
            walkAndTalk(npc);
            return;
        }
    ...
    private void combatGuide()
    {
        ...
        if (progress <= 370) {
            walkToArea(TutAreas.COMBAT_INSTRUCTOR_AREA);
            walkAndTalk(npc);
    ...
    private void bankerGuide()
    {
        ...
        if (progress == 510) {
            walkToArea(TutAreas.TUT_ISLAND_BANK_AREA);
    ...
    private void prayerGuide()
    {
        ...
        if (progress == 640 || progress == 550 || progress == 540) {
            walkToArea(TutAreas.CHURCH_AREA);
            walkAndTalk(npc);
    ...
    private void mageGuide()
    {
        ...
        if (progress == 610 || progress == 620) {
            WorldPoint targetPoint = npc != null ? npc.getWorldLocation() : randomPoint(TutAreas.TUTORIAL_END_AREA);
    ...
    {
        List<AccountQueueEntry> accounts = new ArrayList<>();
        ...
        for (String line : lines) {
            AccountQueueEntry account = parseAccountQueueLine(line);
    ...
    private boolean walkAndAttackRat()
    {
        ...
        walkToArea(TutAreas.RAT_PIT_AREA);
