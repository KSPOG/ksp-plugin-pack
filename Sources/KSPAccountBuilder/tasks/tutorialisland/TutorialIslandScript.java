...
public class TutorialIslandScript extends Script
{
    ...
    private static final WorldArea QUEST_TUT_AREA = new WorldArea(3083, 3119, 7, 7, 0);
    ...
    private static final WorldArea RAT_PIT_AREA = new WorldArea(3097, 9514, 12, 9, 0);
    private static final WorldArea TUT_ISLAND_BANK_AREA = new WorldArea(3118, 3119, 7, 7, 0);
    private static final WorldArea BOND_INFO_AREA = new WorldArea(3125, 3123, 5, 3, 0);
    private static final WorldArea CHURCH_AREA = new WorldArea(3120, 3103, 8, 8, 0);
    private static final WorldArea TUTORIAL_END_AREA = new WorldArea(3140, 3084, 4, 6, 0);
    private static final WorldArea[] TUTORIAL_AREAS = {
            START_AREA,
            SURVIVAL_AREA,
            COOKING_AREA,
            QUEST_TUT_AREA,
    ...
    {
        int bodyTypeChild = ThreadLocalRandom.current().nextBoolean()
                ? BODY_TYPE_A_CHILD
                : BODY_TYPE_B_CHILD;
    ...
    {
        Rs2NpcModel npc = Microbot.getRs2NpcCache().query().withId(NpcID.GIELINOR_GUIDE).nearest();
        int progress = Microbot.getVarbitPlayerValue(281);
    ...
    private void survivalGuide()
    {
        ...
        if (progress == 10 || progress == 20 || progress == 60) {
            ...
            if (ownFireLocation == null && !Rs2Inventory.contains("Logs")) {
                cutTree();
                return;
            }

            if (ownFireLocation == null || getOwnFire() == null) {
                lightFire();
                return;
            }
    ...
    private boolean talkToSurvivalExpert()
    {
        ...
        Rs2NpcModel npc = Microbot.getRs2NpcCache().query().withId(NpcID.SURVIVAL_EXPERT).nearest();

        if (npc == null) {
            npc = Microbot.getRs2NpcCache().query().withName("Survival Expert").nearest();
        }
    ...
    {
        Rs2NpcModel npc = Microbot.getRs2NpcCache().query().withId(NpcID.MASTER_CHEF).nearest();
        int progress = Microbot.getVarbitPlayerValue(281);
    ...
    {
        Rs2NpcModel npc = Microbot.getRs2NpcCache().query().withId(NpcID.QUEST_GUIDE).nearest();
        int progress = Microbot.getVarbitPlayerValue(281);

        if (progress == 200 || progress == 210) {
            walkToArea(QUEST_TUT_AREA);
    ...
    {
        Rs2NpcModel npc = Microbot.getRs2NpcCache().query().withId(NpcID.MINING_INSTRUCTOR).nearest();
    ...
    {
        Rs2NpcModel npc = Microbot.getRs2NpcCache().query().withId(NpcID.COMBAT_INSTRUCTOR).nearest();
        int progress = Microbot.getVarbitPlayerValue(281);

        if (progress <= 370) {
            walkToArea(COMBAT_INSTRUCTOR_AREA);
            walkAndTalk(npc);
    ...
    {
        Rs2NpcModel npc = Microbot.getRs2NpcCache().query().withId(NpcID.ACCOUNT_GUIDE).nearest();
        int progress = Microbot.getVarbitPlayerValue(281);
    ...
    {
        Rs2NpcModel npc = Microbot.getRs2NpcCache().query().withId(NpcID.BROTHER_BRACE).nearest();
        int progress = Microbot.getVarbitPlayerValue(281);
    ...
    {
        Rs2NpcModel npc = Microbot.getRs2NpcCache().query().withId(NpcID.MAGIC_INSTRUCTOR).nearest();
        int progress = Microbot.getVarbitPlayerValue(281);
    ...
    private boolean castHomeTeleport()
    {
        ...
        sleepUntil(() -> {
            ...
            return point != null && point.getX() >= 3200;
        }, 15_000);
        return true;
    }
    ...
    private boolean performQueuedLogin(AccountQueueEntry account, int world)
    {
        ...
        try {
            ...
            // World selection can fail on some login states; credentials are still worth submitting.
        }
    ...
    private AccountQueueEntry parseAccountQueueLine(String line)
    {
        ...
        int world = parseWorld(parts.length >= 3 ? parts[2].trim() : "");

        if (username.isEmpty() || password.isEmpty()) {
            return null;
        }

        return new AccountQueueEntry(username, password, world);
    }
    ...
    private boolean walkAndAttackRat()
    {
        ...
        if (playerLocation != null && playerLocation.getX() == 3111 && playerLocation.getY() >= 9516 && playerLocation.getY() <= 9519) {
            if (Microbot.getRs2TileObjectCache().query().withId(9719).interact("Open")) {
                sleepUntil(() -> {
                    WorldPoint point = Rs2Player.getWorldLocation();
                    return point != null && point.getY() < 9516;
                }, 3000);
                return false;
            }
        }

        walkToArea(RAT_PIT_AREA);
        Rs2NpcModel rat = Microbot.getRs2NpcCache().query().withName("Giant rat").nearest();

        if (rat == null || rat.getWorldLocation() == null || !Rs2Walker.canReach(rat.getWorldLocation())) {
            return false;
        }
    ...
    private void closeEquipmentStats()
    {
        ...
        for (Widget dynamicWidgetOption : widgetOptions.getDynamicChildren()) {
            ...
            if (actionsText != null && Arrays.stream(actionsText).anyMatch(action -> action.equalsIgnoreCase("close"))) {
    ...
    private void handleBankSpaceAndPollBooth()
    {
        ...
        Microbot.getRs2TileObjectCache().query().interact(26815);
    ...
    private void lightFire()
    {
        ...
        WorldPoint expectedFireLocation = fireLocation;
        sleepUntil(() -> !Rs2Inventory.hasItem("Logs") && getFireAt(expectedFireLocation) != null, 5000);

        if (getFireAt(expectedFireLocation) != null) {
    ...
    private void cookShrimpOnOwnFire()
    {
        ...
        sleepUntil(() -> !Rs2Inventory.hasItem(ItemID.RAW_SHRIMPS_2514) || Microbot.getVarbitPlayerValue(281) > 90, 5000);
    }
    ...
    private TileObject getFireAt(WorldPoint location)
    {
        ...
        if (tileObject == null || tileObject.getId() != ObjectID.FIRE_26185) {
            return null;
        }
    ...
    public TutState getTutState()
    {
        ...
        if (currentStatus == null || currentStatus.isEmpty()) {
            return isOnTutorialIsland() ? TutState.RUNNING : TutState.IDLE;
        }
        ...
        try {
            return TutState.valueOf(normalizedStatus);
    ...
    public static boolean isOnTutorialIsland()
    {
        ...
        if (playerLocation != null) {
            for (WorldArea area : TUTORIAL_AREAS) {
    ...
    private static class AccountQueueEntry
    {
